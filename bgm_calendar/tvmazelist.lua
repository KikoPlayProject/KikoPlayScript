...
settings = {
    ["schedule_info_type_02"]={
        ...
        ["default"]="scripted",
        ["desc"]="时间表中出现的剧集、节目类型。\n"..
                "scripted：包含剧本类、动画、纪录等 (默认)。scripted_variety：包含前者，以及综艺、游戏、颁奖、真人节目等。\n"..
                "scripted_show：包含前者，以及访谈、讨论节目等。tv_show：包括前者，以及体育、新闻节目等，即所有节目。",
...
Translation["zh-CN"] = {
    ["show_type"]= {
        ["Scripted"]= "剧本类", ["Animation"]= "动画", ["Documentary"]= "纪录", ["Reality"]= "真人节目", ["Variety"]= "综艺", ["Game Show"]= "游戏", ["Award Show"]= "颁奖", ["Talk Show"]= "访谈", ["Panel Show"]= "讨论节目", ["Sports"]= "体育", ["News"]= "新闻",
        [""]= "其他", ["Unknown"]= "未知",
    },
}
    ...
    for _,ep in ipairs(objCs) do
        ...
        if (ep.show or{}).type~="Scripted" and (ep.show or{}).type~="Animation" and (ep.show or{}).type~="Documentary" then
            if schedule_info_type_tmp=="scripted" then
                goto continue_gbl_wii
            elseif (ep.show or{}).type~="Reality" and (ep.show or{}).type~="Variety" and (ep.show or{}).type~="Game Show" and (ep.show or{}).type~="Award Show" then
                if schedule_info_type_tmp=="scripted" or schedule_info_type_tmp=="scripted_variety" then
                    goto continue_gbl_wii
                elseif (ep.show or{}).type~="Talk Show" and (ep.show or{}).type~="Panel Show" then
